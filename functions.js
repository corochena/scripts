...
function scriptSize(s) {
    return s.ranges.reduce((count, [from, to]) => count + to - from, 0)
}

let biggest = SCRIPTS.reduce((big, s) => scriptSize(s) > scriptSize(big) ? s : big)
console.log(biggest)
console.log(scriptSize(biggest))

// Find the average year of origin for living and dead scripts
let live = SCRIPTS.filter(s => s.living)
let dead = SCRIPTS.filter(s => !s.living)
function avg(a) {
  return a.reduce((a1, a2) => a1 + a2) / a.length
}
console.log(Math.round(avg(live.map(s => s.year))))
console.log(Math.round(avg(dead.map(s => s.year))))

// Given a character code, find the corresponding script
function findScript1(code) {
...
console.log(findScript(121).name)
...
*/
function countBy(a, groupName) {
  let result = []
  for (let elem of a) {
    let name = groupName(elem)
    let index = result.findIndex(group => group.name == name)
    if (index == -1) result.push({name, count: 1})
    else result[index].count++
  }
  return result
}

// Using countBy write a function that tells us which scripts are used in a 
// piece of text

function textScripts(text) {
  let result = countBy(text, char => {
    findScript(char.codePointAt(0)).name
  })
  return result
}
