...
let content = ""
scriptNames.forEach(name => content += "<option value='" + name + "'>" + name + "</option>")
select.innerHTML = content

select.addEventListener('change', e => {
  document.querySelector("p").innerHTML = scriptChars(e.target.value)
})

function scriptChars(name) {
  let ranges = SCRIPTS.filter(s => s.name == name)[0].ranges
  let scriptString = ""
  ranges.forEach(([from, to]) => {
    for (let code=from; code<to; code++)
      scriptString += String.fromCodePoint(code) + " "
  })
  return scriptString
}